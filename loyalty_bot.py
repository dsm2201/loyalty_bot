...
def format_client_cabinet(client, phone: str) -> str:
    ...
    text = (
        ...
        f"Телефон: {phone}\n"
        f"Всего покупок на сумму: {turnover:.0f}₽\n"
        f"Бонусный счёт: {bonus:.0f} бонусов\n\n"
        f"{lvl_text}\n"
        f"{next_level_line}\n\n"
        "Каждая печать фото, ксерокс, скан или другая услуга в ателье — это ещё один шаг к новым бонусам.\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Нажмите кнопку «Открыть личный кабинет», чтобы узнать свой уровень и количество бонусов.\n\n"
        "Продолжая использовать бота и нажимая кнопку «Открыть личный кабинет», "
        "вы даёте согласие на обработку персональных данных (необходимо по 152‑ФЗ) "
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cabinet_open":
        ...
        await query.message.reply_text(
            ...
            "чтобы Вы могли получать и списывать бонусы."
        )
        return
    ...
    if data == "pdn_info":
        await query.message.reply_text(
            ...
            "Отправляя номер телефона и используя бота программы лояльности, "
            "вы даёте согласие на обработку ваших персональных данных в целях ведения "
            "программы лояльности, учёта покупок, начисления и списания бонусов, "
            "а также для обратной связи по услугам фото-ателье.\n\n"
            "Мы обрабатываем только минимально необходимые данные: номер телефона, "
            "Telegram ID и ник, историю операций по бонусному счёту. "
    ...
    if data == "privacy_policy":
        await query.message.reply_text(
            ...
            "Мы храним номер телефона, Telegram ID/ник и историю операций по бонусам "
            "в защищённых сервисах (в том числе Google Sheets) с ограниченным доступом. "
            "Данные используются только для работы программы лояльности «Фото Химки», "
            "отображения личного кабинета, истории покупок и начисления/списания бонусов.\n\n"
            "Вы можете запросить уточнение, блокирование или удаление своих данных, "
            "написав нам в бота или обратившись к администратору фото-ателье. "
    ...
    if data == "send_file":
        ...
        await query.message.reply_text(
            ...
            "Мы перешлём его администратору."
        )
        return
    ...
    if data == "leave_review":
        ...
        await query.message.reply_text(
            "Хочешь +100 бонусов? 🔥\n\n"
            f"1) Перейдиnt по ссылке и оставь приятный отзыв о «Фото Химки» на Яндекс.Картах:\n{YANDEX_REVIEW_URL}\n\n"
            "2) Сделайте скриншот своего отзыва.\n"
            "3) Пришлите скрин сюда в ответ на это сообщение.\n\n"
            "Как только отзыв появится на Яндексе, мы проверим и начислим Вам 100 бонусов,"
            "Которые в дальнейшем можно будет потратить на услуги в Фото Химки."
        )
        return
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_review_screenshot"):
        ...
        await update.message.reply_text(
            "Спасибо за отзыв! 🙌\n"
            "Как только он появится на Яндекс.Картах, мы это проверим и начислим +100 бонусов на Ваш счёт."
        )
        return
